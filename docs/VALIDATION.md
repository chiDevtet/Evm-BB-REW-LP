...
Native `eth_call` discards its writes, so a successful call does not prove persistent multi-round behavior, production holder eligibility or actual payment receipts. The independent fork gate is `npm run test:fork`; its report is generated in `artifacts/robinhood-fork-report.json`. **The multi-round fork gate has NOT passed.** The official RPC returned HTTP 405, PublicNode rejected pinned archive requests without a personal token, and dRPC attempts hit timeouts. A pinned Anvil run reached block 70,802,320, checked native upstream ArbSys and installed the local clock adapter, then stopped with an RPC call failure before completing the legacy recovery or processing rounds. The [last retry report](validation/robinhood-fork-2026-09-23.json) records a timeout. Keep the PR in draft until this gate completes against a reliable archive RPC.

Anvil runs Cancun so real V4 transient-storage opcodes are supported; the engine compiler still targets Shanghai. Anvil's two-method ArbSys adapter is explicitly disclosed in that gate, and native ArbSys is checked separately against the upstream chain. The adapter does not emulate all ArbOS behavior.
